...
const createApp =  (name, id, location, basemap, color) => {
    
    let _name = id.toString();
    console.log('Creating app: ' + _name);

    // Function that creates the folder structure and the app
    const createAppStructure = function () {
        
        fs.mkdir(path.join(globalPath, 'src', 'maps', _name), function () {
            console.log('Folder created');
        });

        // Create App
        fs.readFile(globalPath + "/src/template/app.html", function (err, data) {
            ...
            fs.appendFile(path.join(globalPath, 'src', 'maps', _name, 'index.html'), dataToCopy, function (err) {
                if (err) throw err;
...
const saveData = (json) => {
    fs.readFile(path.join(globalPath, 'src', 'data', 'data.json'), (err, data) => {
        if (err) throw err;
        let apps = JSON.parse(data);
        apps.apps.push(json);
        fs.writeFileSync(path.join(globalPath, 'src', 'data', 'data.json'), JSON.stringify(apps));
    });
};

const getData = async () => {
    return await fs.readFileSync(path.join(globalPath, 'src', 'data', 'data.json'), (err, data) => {
        if (err) throw err;
        let apps = JSON.parse(data);
        return apps;
    });
